...
void Encoding<POS_DIM, NR_FEAT_PER_LEVEL>::check_positions(
  ...
  CHECK(positions_raw.scalar_type() == at::kFloat)
      << "positions should be of type float";
  CHECK(positions_raw.dim() == 2)
      << "positions should have dim 2 correspondin to HW. However it has sizes"
      << positions_raw.sizes();
...
torch::Tensor Encoding<POS_DIM, NR_FEAT_PER_LEVEL>::forward(
  ...
  forward_gpu<POS_DIM, NR_FEAT_PER_LEVEL>
      <<<blocks, BLOCK_SIZE, 0, at::cuda::getCurrentCUDAStream()>>>(
          ...
          input.m_require_lattice_values_grad, input.m_require_positions_grad);
...
Encoding<POS_DIM, NR_FEAT_PER_LEVEL>::backward(
  ...
  if (input.m_require_lattice_values_grad) {
    // lattice_values_monolithic_grad=torch::zeros({ nr_resolutions, capacity,
    // val_dim },  torch::dtype(torch::kFloat32).device(torch::kCUDA, 0)  );
  ...
  if (input.m_require_positions_grad) {
    // positions_grad=torch::empty({ nr_resolutions, nr_positions, pos_dim },
    // torch::dtype(torch::kFloat32).device(torch::kCUDA, 0)  );
    // positions_grad=torch::zeros({ nr_positions, pos_dim },
    // torch::dtype(torch::kFloat32).device(torch::kCUDA, 0)  );
    positions_grad =
        torch::zeros({pos_dim, nr_positions},
                     torch::dtype(torch::kFloat32).device(torch::kCUDA, 0));
  } else {
    // positions_grad=torch::empty({ 1,1,1 },
    // torch::dtype(torch::kFloat32).device(torch::kCUDA, 0)  );
  ...
  // assumes lattice_values_monolithic_grad=torch::zeros({ nr_resolutions,
  // val_dim, capacity },  torch::dtype(torch::kFloat32).device(torch::kCUDA, 0)
  // );
  backward_gpu<POS_DIM, NR_FEAT_PER_LEVEL>
      <<<blocks, BLOCK_SIZE_BACK, 0, at::cuda::getCurrentCUDAStream()>>>(
          ...
          positions_grad
              .packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
          m_fixed_params.m_concat_points, input.m_require_lattice_values_grad,
...
Encoding<POS_DIM, NR_FEAT_PER_LEVEL>::double_backward_from_positions(
  ...
  // const dim3 blocks = { (unsigned int)div_round_up(nr_positions,
  // BLOCK_SIZE_DOUBLE_BACK), (unsigned
  // int)(nr_resolutions+nr_resolutions_extra), 1 }; //the blocks are executed
  // in order, first the blocks for the first resolution, then the second and so
  // on double_backward_from_positions_gpu<POS_DIM, NR_FEAT_PER_LEVEL><<<blocks,
  // BLOCK_SIZE_DOUBLE_BACK>>>(
  //     nr_positions,
  //     capacity,
  //     nr_resolutions,
...
// template<uint32_t POS_DIM, uint32_t NR_FEAT_PER_LEVEL>
// void Encoding<POS_DIM, NR_FEAT_PER_LEVEL>::copy_to_constant_mem(const
// EncodingFixedParams& fixed_params){

//     CHECK(fixed_params.m_random_shift_per_level.numel()<256) << "We reserved
//     a maximum of 256 for the constant memory and we exceding that";
//     CHECK(fixed_params.m_scale_factor.numel()<256) << "We reserved a maximum
//     of 256 for the constant memory and we exceding that";

//     //the onyl way to populate the constant memory is with cudaMemcpyToSymbol
//     whcih copies from cpu to gpu Tensor
//     random_shift_cpu=fixed_params.m_random_shift_per_level.view({-1}).cpu();
//     Tensor scale_factor_cpu=fixed_params.m_scale_factor.view({-1}).cpu();

//     cudaMemcpyToSymbol(random_shift_constant, random_shift_cpu.data_ptr(),
//     sizeof(float) * random_shift_cpu.numel());
//     cudaMemcpyToSymbol(scale_factor_constant, scale_factor_cpu.data_ptr(),
//     sizeof(float) * scale_factor_cpu.numel());
// }
