...
// In order to create an encoding we need to return a templated version of it
// with the create_encoding factory. we create a baseclass with no templates so
// we can return that one
...
// line here AND adding the specialization at Encoding.cu this functions
...
inline std::shared_ptr<EncodingBase> create_encoding_template_pos_dim(
  ...
  switch (nr_feat_per_level) {
    ...
    // TODO cannot add more values at the moment because we have harcoded the
    // CUDA kernels for 2 features per lvl
    default:
      throw std::runtime_error{
          "Encoding: nr_feat_per_level must be 2 since other values are not "
          "yet implemented"};
  }
}
...
// In order to expose the Encoding to Pybind we would need an explicit
// specialization for every encoding template. Instead we expose this object and
// internally here we create all the encoding and template stuff and expose this
