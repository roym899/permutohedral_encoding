...
class ModelForPerformance(torch.nn.Module):
    def __init__(
        self, input_channels, log2_hashmap_size, nr_lattice_features, nr_resolutions
    ):
        ...
        self.encoding_voxel = tcnn.Encoding(
            n_input_dims=input_channels, encoding_config=config_encoding
        )
        ...
        self.encoding_permuto = permuto_enc.PermutoEncoding(
...
def inference(model, pts, lattice_type, iters_warmup, iters_to_measure):
    ...
    with torch.set_grad_enabled(False):
        ...
        with torch.set_grad_enabled(False):
            ...
            mean_inference_throughput = int(
                mean_inference_throughput / nr_iterations_executed
            )
            print(
                ...
                pos_dim,
                " ",
                f"{mean_inference_throughput:,}",
            )


def training_backward_towards_lattice(
    model, pts, lattice_type, iters_warmup, iters_to_measure, optimizer
):
    nr_points = pts.shape[0]
    pos_dim = pts.shape[1]
    model.train()
    with torch.set_grad_enabled(True):
        ...
        for iter_nr in range(iters_to_measure):
            ...
            if iter_nr > iters_warmup:
                ...
                nr_iterations_executed += 1
        mean_training_throughput = int(
            mean_training_throughput / nr_iterations_executed
        )
        print(
            "mean_training_backwards_lattice_throughput ",
            lattice_type,
            " pos_dim ",
            pos_dim,
            " ",
            f"{mean_training_throughput:,}",
        )
        # print("loss is", loss.item())


def training_backward_towards_lattice_and_pos(
    model, pts, lattice_type, iters_warmup, iters_to_measure, optimizer
):
    nr_points = pts.shape[0]
    pos_dim = pts.shape[1]
    ...
    with torch.set_grad_enabled(True):
        ...
        print(
            "mean_training_backwards_lattice_and_pos_throughput ",
            lattice_type,
            " pos_dim ",
            pos_dim,
            " ",
...
def measure_performance(nr_points, pos_dim, lattice_type):
    ...
    # model
    model = ModelForPerformance(
        pos_dim, log2_hashmap_size, nr_lattice_features, nr_resolutions
    )
    ...
    training_backward_towards_lattice(
        model, pts, lattice_type, iters_warmup, iters_to_measure, optimizer
    )
    training_backward_towards_lattice_and_pos(
        model, pts, lattice_type, iters_warmup, iters_to_measure, optimizer
    )
